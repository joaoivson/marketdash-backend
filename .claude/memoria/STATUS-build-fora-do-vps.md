...
| 3 | Backend: Dockerfile multi-stage | targets `worker` e `api`, `api` por último | eu | 🔄 | — | — |
| 4 | Backend: `version` no /health e no boot | `APP_VERSION` + `/`, `/health`, worker log | eu | ⬜ | ⬜ | — |
| 5 | Backend: `deploy-imagem.sh` + `confirmar-versao.sh` | trava `build_pack`, PATCH tag, poll até `finished` | eu | ⬜ | ⬜ | — |
| 6 | Backend: workflows prod/hml | build+push GHCR, gate de aprovação em prod | eu | ⬜ | ⬜ | — |
| 7 | Frontend: Dockerfile com ARGs VITE_* | guarda contra tela branca + `version.json` | eu | ⬜ | ⬜ | — |
| 8 | Frontend: workflows | Variables por ambiente, hash exato do bundle | eu | ⬜ | ⬜ | — |
| 9 | Retenção no GHCR | workflow semanal, mantém 15 versões | eu | ⬜ | — | — |
