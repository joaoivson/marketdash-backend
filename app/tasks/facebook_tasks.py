...
def sync_facebook_user_task(self, user_id: int, trigger: str = "manual"):
    ...
    try:
        ...
        processed = asyncio.run(svc.sync_user(user_id, db, trigger=trigger))
...
@celery_app.task
def sync_all_facebook_users_task(trigger: str = "cron"):
